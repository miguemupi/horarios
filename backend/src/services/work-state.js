...
async function snapshot(client, dayId) {
  ...
  const tasks = await client.query(
    `SELECT id, client_entry_id, business_name, description, material,
...
export async function currentWorkDay(sessionUser) {
  ...
  const result = await transaction(async (client) => {
    ...
    const task = await client.query(
      ...
      [dayId, business.rows[0].id, business.rows[0].name, input.work.trim(), input.material?.trim() || '', start,
        input.endTime, durationSeconds, position, input.clientEntryId],
    );
    await client.query(
